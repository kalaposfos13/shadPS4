// SPDX-FileCopyrightText: Copyright 2025 shadPS4 Emulator Project
// SPDX-License-Identifier: GPL-2.0-or-later

#include "common/logging/log.h"
#include "core/libraries/error_codes.h"
#include "core/libraries/libs.h"
#include "core/libraries/json2/json2.h"

namespace Libraries::Json2 {

s32 PS4_SYSV_ABI _ZN3sce4Json11Initializer10initializeEPKNS0_13InitParameterE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json11Initializer10initializeEPKNS0_14InitParameter2E() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json11Initializer24setAllocatorInfoCallBackEPFviNS0_9ValueTypeEPvES3_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json11Initializer27setGlobalNullAccessCallbackEPFRKNS0_5ValueENS0_9ValueTypeEPS3_PvES7_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json11Initializer28setGlobalSpecialFloatHandlerEPFKNS0_5ValueENS0_12FunctionTypeEdPS3_PvPbES6_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json11Initializer28setGlobalTypeMismatchHandlerEPFRKNS0_5ValueENS0_9ValueTypeEPS3_S6_PvES7_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json11Initializer36setGlobalElementAccessFailureHandlerEPFviPKNS0_6StringEPKmPKNS0_5ValueES9_PvESA_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json11Initializer9terminateEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json11InitializerC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json11InitializerC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json11InitializerD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json11InitializerD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json12MemAllocator11notifyErrorEimPv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json12MemAllocatorC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json12MemAllocatorD0Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json12MemAllocatorD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json12MemAllocatorD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json14InitParameter212setAllocatorEPNS0_12MemAllocatorEPv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json14InitParameter217setFileBufferSizeEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json14InitParameter225setSpecialFloatFormatTypeENS0_22SpecialFloatFormatTypeE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json14InitParameter2C1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json14InitParameter2C2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json17InitParameterRttiC1EPNS0_14AllocParamRttiEPvm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json17InitParameterRttiC2EPNS0_14AllocParamRttiEPvm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json18InitParameterRtti216setAllocatorRttiEPNS0_14AllocParamRttiEPv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json19InternalInitializer19terminate_staticlibEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json19InternalInitializer20initialize_staticlibEPKNS0_13InitParameterENS1_10InitOptionE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json19InternalInitializerC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json19InternalInitializerC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json19InternalInitializerD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json19InternalInitializerD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json4FreeEPv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array10push_frontERKNS0_5ValueE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array5clearEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array5eraseERKNS1_8iteratorE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array6insertERKNS1_8iteratorERKNS0_5ValueE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iterator7advanceEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratoraSERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorC1ERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorC2ERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorppEi() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8iteratorppEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array8pop_backEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array9pop_frontEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Array9push_backERKNS0_5ValueE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ArrayaSERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ArrayC1ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ArrayC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ArrayC2ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ArrayC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ArrayD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ArrayD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value10referArrayEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value10referValueEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value10referValueERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value10s_nullboolE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value10s_nullrealE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value11referObjectEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value11referStringEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value11s_nullarrayE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value12referBooleanEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value12referIntegerEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value12s_nullobjectE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value12s_nullstringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value13referUIntegerEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value13s_nullintegerE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value14s_nulluintegerE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value21setNullAccessCallBackEPFRKS1_NS0_9ValueTypeEPS2_PvES6_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json5Value22setSpecialFloatHandlerEPFKS1_NS0_12FunctionTypeEdPS2_PvPbES5_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value22setTypeMismatchHandlerEPFRKS1_NS0_9ValueTypeEPS2_S5_PvES6_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI
_ZN3sce4Json5Value30setElementAccessFailureHandlerEPFviPKNS0_6StringEPKmPKS1_S8_PvES9_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setEb() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setEd() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setEl() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setENS0_9ValueTypeE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setEPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setERKNS0_5ArrayE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setERKNS0_6ObjectE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value3setERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value4swapERS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value5clearEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value9referRealEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value9serializeEPFiRNS0_6StringEPvES4_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value9serializeERNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5Value9serializeERNS0_6StringEPFiS3_PvES4_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueaSERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1Eb() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1Ed() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1El() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1Em() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1ENS0_9ValueTypeE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1EPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1ERKNS0_5ArrayE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1ERKNS0_6ObjectE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1ERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2Eb() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2Ed() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2El() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2Em() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2ENS0_9ValueTypeE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2EPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2ERKNS0_5ArrayE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2ERKNS0_6ObjectE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2ERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json5ValueD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6MallocEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object4PairC1ERKNS0_6StringERKNS0_5ValueE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object4PairC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object4PairC2ERKNS0_6StringERKNS0_5ValueE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object4PairC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object4PairD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object4PairD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object5clearEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object5eraseERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object6insertERKNS1_4PairE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iterator7advanceEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratoraSERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorC1ERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorC2ERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorppEi() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Object8iteratorppEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectaSERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectC1ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectC2ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6ObjectixERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Parser5parseERNS0_5ValueEPFiRcPvES5_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Parser5parseERNS0_5ValueEPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6Parser5parseERNS0_5ValueEPKcm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6String4nposE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6String5clearEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6String6appendEPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6String6appendEPKcm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6String6appendERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6String6resizeEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6String7reserveEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringaSERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringC1EPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringC1ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringC1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringC2EPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringC2ERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringC2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringD1Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringD2Ev() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringpLEh() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZN3sce4Json6StringpLEPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json14InitParameter211getUserDataEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json14InitParameter212getAllocatorEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json14InitParameter217getFileBufferSizeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json14InitParameter225getSpecialFloatFormatTypeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array3endEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array4backEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array4sizeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array5beginEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array5emptyEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array5frontEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array8iteratordeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array8iteratorneERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Array8iteratorptEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value10getBooleanEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value10getIntegerEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value11getUIntegerEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value5countEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value7getRealEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value7getTypeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value8getArrayEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value8getValueEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value8getValueERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value8toStringERNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value9getObjectEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5Value9getStringEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5ValuecvbEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5ValueixEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5ValueixEPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json5ValueixERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object3endEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object4findERKNS0_6StringE() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object4sizeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object5beginEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object5emptyEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object8iteratordeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object8iteratoreqERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object8iteratorneERKS2_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6Object8iteratorptEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String2atEm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String4findEcm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String4findEPKcm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String4findEPKcmm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String4findERKS1_m() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String4sizeEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String5c_strEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String5emptyEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String5rfindEcm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String5rfindEPKcm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String5rfindEPKcmm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String5rfindERKS1_m() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String6lengthEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String6substrEmm() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String7compareEPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String7compareERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6String8capacityEv() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6StringeqEPKc() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

s32 PS4_SYSV_ABI _ZNK3sce4Json6StringeqERKS1_() {
    LOG_ERROR(Lib_Json, "(STUBBED) called");
    return ORBIS_OK;
}

void RegisterLib(Core::Loader::SymbolsResolver* sym) {
    LIB_FUNCTION("Cxwy7wHq4J0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer10initializeEPKNS0_13InitParameterE);
    LIB_FUNCTION("IXW-z8pggfg", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer10initializeEPKNS0_14InitParameter2E);
    LIB_FUNCTION("R996H0YIhiQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer24setAllocatorInfoCallBackEPFviNS0_9ValueTypeEPvES3_);
    LIB_FUNCTION("+drDFyAS6u4", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer27setGlobalNullAccessCallbackEPFRKNS0_5ValueENS0_9ValueTypeEPS3_PvES7_);
    LIB_FUNCTION("i1393UBWu1U", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer28setGlobalSpecialFloatHandlerEPFKNS0_5ValueENS0_12FunctionTypeEdPS3_PvPbES6_);
    LIB_FUNCTION("tck9g-yOqPg", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer28setGlobalTypeMismatchHandlerEPFRKNS0_5ValueENS0_9ValueTypeEPS3_S6_PvES7_);
    LIB_FUNCTION("j8Q7SBgMm28", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer36setGlobalElementAccessFailureHandlerEPFviPKNS0_6StringEPKmPKNS0_5ValueES9_PvESA_);
    LIB_FUNCTION("PR5k1penBLM", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11Initializer9terminateEv);
    LIB_FUNCTION("cK6bYHf-Q5E", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11InitializerC1Ev);
    LIB_FUNCTION("6qFqND4iwPA", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11InitializerC2Ev);
    LIB_FUNCTION("RujUxbr3haM", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11InitializerD1Ev);
    LIB_FUNCTION("qQZGdYkEytk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json11InitializerD2Ev);
    LIB_FUNCTION("i-XwZjw0OOY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json12MemAllocator11notifyErrorEimPv);
    LIB_FUNCTION("-hJRce8wn1U", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json12MemAllocatorC2Ev);
    LIB_FUNCTION("ypazoTrKYhE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json12MemAllocatorD0Ev);
    LIB_FUNCTION("O1uwoqazS2A", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json12MemAllocatorD1Ev);
    LIB_FUNCTION("OcAgPxcq5Vk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json12MemAllocatorD2Ev);
    LIB_FUNCTION("I2QC8PYhJWY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json14InitParameter212setAllocatorEPNS0_12MemAllocatorEPv);
    LIB_FUNCTION("Eu95jmqn5Rw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json14InitParameter217setFileBufferSizeEm);
    LIB_FUNCTION("WVZBP4IyM+E", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json14InitParameter225setSpecialFloatFormatTypeENS0_22SpecialFloatFormatTypeE);
    LIB_FUNCTION("WSOuge5IsCg", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json14InitParameter2C1Ev);
    LIB_FUNCTION("GvGvswb0v34", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json14InitParameter2C2Ev);
    LIB_FUNCTION("6i18OJSvFWk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json17InitParameterRttiC1EPNS0_14AllocParamRttiEPvm);
    LIB_FUNCTION("Mp8DPb3M4I0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json17InitParameterRttiC2EPNS0_14AllocParamRttiEPvm);
    LIB_FUNCTION("W72B9ylU2JA", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json18InitParameterRtti216setAllocatorRttiEPNS0_14AllocParamRttiEPv);
    LIB_FUNCTION("dvp2zGSQzMs", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json19InternalInitializer19terminate_staticlibEv);
    LIB_FUNCTION("BPSPc3pUMJo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json19InternalInitializer20initialize_staticlibEPKNS0_13InitParameterENS1_10InitOptionE);
    LIB_FUNCTION("WntRKhYTmZw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json19InternalInitializerC1Ev);
    LIB_FUNCTION("2n+fxNdSZfs", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json19InternalInitializerC2Ev);
    LIB_FUNCTION("yudT4-bglko", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json19InternalInitializerD1Ev);
    LIB_FUNCTION("caP-y4fH-zQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json19InternalInitializerD2Ev);
    LIB_FUNCTION("bnk+edDbqMk", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json4FreeEPv);
    LIB_FUNCTION("MQxDxBzWz40", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array10push_frontERKNS0_5ValueE);
    LIB_FUNCTION("qVOSuDRHCpA", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5Array5clearEv);
    LIB_FUNCTION("2mbVAtshWRI", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array5eraseERKNS1_8iteratorE);
    LIB_FUNCTION("ixJAz92H1uE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array6insertERKNS1_8iteratorERKNS0_5ValueE);
    LIB_FUNCTION("43MESLndGvQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iterator7advanceEm);
    LIB_FUNCTION("T3WReElX4BY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratoraSERKS2_);
    LIB_FUNCTION("IaIvX3kgyb0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorC1ERKS2_);
    LIB_FUNCTION("TAPTWYaYf8Q", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorC1Ev);
    LIB_FUNCTION("Hdc3DSIsG2I", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorC2ERKS2_);
    LIB_FUNCTION("JSg1MSo7NKE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorC2Ev);
    LIB_FUNCTION("9yLjn46Ypfs", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorD1Ev);
    LIB_FUNCTION("9niNrXsF3Bc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorD2Ev);
    LIB_FUNCTION("GEylwSsiGG8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorppEi);
    LIB_FUNCTION("w5+VCznos5E", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8iteratorppEv);
    LIB_FUNCTION("woec1toi4Xw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array8pop_backEv);
    LIB_FUNCTION("pPs3qZTZQvQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array9pop_frontEv);
    LIB_FUNCTION("zQtLRTqceMY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Array9push_backERKNS0_5ValueE);
    LIB_FUNCTION("w5UbvPLGye0", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ArrayaSERKS1_);
    LIB_FUNCTION("bI5AGFMydrA", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ArrayC1ERKS1_);
    LIB_FUNCTION("JP-PtKMiI1E", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ArrayC1Ev);
    LIB_FUNCTION("6NlbpyIfUuY", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ArrayC2ERKS1_);
    LIB_FUNCTION("t6AHhkWu4mY", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ArrayC2Ev);
    LIB_FUNCTION("HJ8GpRT1aiw", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ArrayD1Ev);
    LIB_FUNCTION("MIyMPw7YTGM", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ArrayD2Ev);
    LIB_FUNCTION("nM5XqdeXFPw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value10referArrayEv);
    LIB_FUNCTION("gLzCc67aTbw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value10referValueEm);
    LIB_FUNCTION("wLsJlmgEIaI", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value10referValueERKNS0_6StringE);
    LIB_FUNCTION("bUw2Go-rxtE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value10s_nullboolE);
    LIB_FUNCTION("GpEG1IcP86s", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value10s_nullrealE);
    LIB_FUNCTION("-NxEk7XLkDY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value11referObjectEv);
    LIB_FUNCTION("m5J28iw-QPs", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value11referStringEv);
    LIB_FUNCTION("zwDiN3-CRTA", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value11s_nullarrayE);
    LIB_FUNCTION("QxjfcfBhEdc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value12referBooleanEv);
    LIB_FUNCTION("R0ac5QOdlpo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value12referIntegerEv);
    LIB_FUNCTION("3YODFwSqgtc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value12s_nullobjectE);
    LIB_FUNCTION("24kDIV0aDzs", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value12s_nullstringE);
    LIB_FUNCTION("Nx5tez4siL4", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value13referUIntegerEv);
    LIB_FUNCTION("dJSD90mnj1w", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value13s_nullintegerE);
    LIB_FUNCTION("xxBnIBiUVzY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value14s_nulluintegerE);
    LIB_FUNCTION("Xbl-LYVFNEE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value21setNullAccessCallBackEPFRKS1_NS0_9ValueTypeEPS2_PvES6_);
    LIB_FUNCTION("bJWBpvGCIJU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value22setSpecialFloatHandlerEPFKS1_NS0_12FunctionTypeEdPS2_PvPbES5_);
    LIB_FUNCTION("ukQodCmg99s", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value22setTypeMismatchHandlerEPFRKS1_NS0_9ValueTypeEPS2_S5_PvES6_);
    LIB_FUNCTION("6tTbrt1O1SY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value30setElementAccessFailureHandlerEPFviPKNS0_6StringEPKmPKS1_S8_PvES9_);
    LIB_FUNCTION("5yHuiWXo2gg", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5Value3setEb);
    LIB_FUNCTION("BSmWDIkV4w4", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5Value3setEd);
    LIB_FUNCTION("QxVVYhP-mvg", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5Value3setEl);
    LIB_FUNCTION("SIe1ZmW7e7s", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5Value3setEm);
    LIB_FUNCTION("IKQimvG9Wqs", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value3setENS0_9ValueTypeE);
    LIB_FUNCTION("n6FC+l9DU70", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5Value3setEPKc);
    LIB_FUNCTION("195ad-jAsTU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value3setERKNS0_5ArrayE);
    LIB_FUNCTION("dFCphqnd+a4", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value3setERKNS0_6ObjectE);
    LIB_FUNCTION("6l3Bv2gysNc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value3setERKNS0_6StringE);
    LIB_FUNCTION("XL8+BUqjB1w", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value3setERKS1_);
    LIB_FUNCTION("9PqrCLfKZZI", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value4swapERS1_);
    LIB_FUNCTION("FIjXN2TkuTs", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5Value5clearEv);
    LIB_FUNCTION("t7UEJrhojhk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value9referRealEv);
    LIB_FUNCTION("PyMbL9F2oBY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value9serializeEPFiRNS0_6StringEPvES4_);
    LIB_FUNCTION("R7FDWtcN6f8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value9serializeERNS0_6StringE);
    LIB_FUNCTION("l1URiQJlSoU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5Value9serializeERNS0_6StringEPFiS3_PvES4_);
    LIB_FUNCTION("4zrm6VrgIAw", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueaSERKS1_);
    LIB_FUNCTION("UeuWT+yNdCQ", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC1Eb);
    LIB_FUNCTION("sOmU4vnx3s0", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC1Ed);
    LIB_FUNCTION("0lLK8+kDqmE", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC1El);
    LIB_FUNCTION("x4AUdbhpRB0", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC1Em);
    LIB_FUNCTION("CbrT3dwDILo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC1ENS0_9ValueTypeE);
    LIB_FUNCTION("b9V6fmppLXY", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC1EPKc);
    LIB_FUNCTION("iZeYfOxtMRg", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC1ERKNS0_5ArrayE);
    LIB_FUNCTION("3xUXnmUkXfo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC1ERKNS0_6ObjectE);
    LIB_FUNCTION("sZIoMRGO+jk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC1ERKNS0_6StringE);
    LIB_FUNCTION("fSb2oQTNrgA", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC1ERKS1_);
    LIB_FUNCTION("qBMjqyBn3OM", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC1Ev);
    LIB_FUNCTION("OK4Ot0ue7J0", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC2Eb);
    LIB_FUNCTION("23nm4oXHlfI", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC2Ed);
    LIB_FUNCTION("EjxUpzR2Yx8", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC2El);
    LIB_FUNCTION("cqucBPSeVDA", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC2Em);
    LIB_FUNCTION("PWm9MyJJVqU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC2ENS0_9ValueTypeE);
    LIB_FUNCTION("+hHa4QYtLY8", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC2EPKc);
    LIB_FUNCTION("VH9dsKpQTmk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC2ERKNS0_5ArrayE);
    LIB_FUNCTION("JXlrCLLCLmc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC2ERKNS0_6ObjectE);
    LIB_FUNCTION("veNviXt6wC0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json5ValueC2ERKNS0_6StringE);
    LIB_FUNCTION("TZyuFeGuw9Y", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC2ERKS1_);
    LIB_FUNCTION("-wa17B7TGnw", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueC2Ev);
    LIB_FUNCTION("WTtYf+cNnXI", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueD1Ev);
    LIB_FUNCTION("0eUrW9JAxM0", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json5ValueD2Ev);
    LIB_FUNCTION("Xzy0onDzSAc", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6MallocEm);
    LIB_FUNCTION("+hYsagaV-Ug", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object4PairC1ERKNS0_6StringERKNS0_5ValueE);
    LIB_FUNCTION("-uQnUEENfpk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object4PairC1Ev);
    LIB_FUNCTION("lt3J4dPvEtM", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object4PairC2ERKNS0_6StringERKNS0_5ValueE);
    LIB_FUNCTION("Wu-urs6MMpQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object4PairC2Ev);
    LIB_FUNCTION("Q0ng4sWj2E8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object4PairD1Ev);
    LIB_FUNCTION("TM0Sj+Ebins", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object4PairD2Ev);
    LIB_FUNCTION("oH8aBmLU+fc", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6Object5clearEv);
    LIB_FUNCTION("3ZhL-MkICwI", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object5eraseERKNS0_6StringE);
    LIB_FUNCTION("ol7KhQl8kfw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object6insertERKNS1_4PairE);
    LIB_FUNCTION("h-V8r4EF7bo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iterator7advanceEm);
    LIB_FUNCTION("w7nqTT3zgxU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratoraSERKS2_);
    LIB_FUNCTION("JATlaeBu+lU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorC1ERKS2_);
    LIB_FUNCTION("a2Zv3idT0sU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorC1Ev);
    LIB_FUNCTION("r5CShmcua+M", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorC2ERKS2_);
    LIB_FUNCTION("OxO+V0ouoS4", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorC2Ev);
    LIB_FUNCTION("hoINmSMlYjI", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorD1Ev);
    LIB_FUNCTION("patTzc7zBjw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorD2Ev);
    LIB_FUNCTION("dIVxrrdpS9M", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorppEi);
    LIB_FUNCTION("DlWmn2ZQuWY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Object8iteratorppEv);
    LIB_FUNCTION("urOpESTBZmo", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6ObjectaSERKS1_);
    LIB_FUNCTION("a+W7HHlwpBs", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6ObjectC1ERKS1_);
    LIB_FUNCTION("OJPTonqdg0I", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6ObjectC1Ev);
    LIB_FUNCTION("tSRqQ-+Y4PE", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6ObjectC2ERKS1_);
    LIB_FUNCTION("SkKczWHdklc", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6ObjectC2Ev);
    LIB_FUNCTION("5JmzZt8twAo", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6ObjectD1Ev);
    LIB_FUNCTION("1siRBursJjw", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6ObjectD2Ev);
    LIB_FUNCTION("ERuf9y0DY84", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6ObjectixERKNS0_6StringE);
    LIB_FUNCTION("itqj2YmuAa8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Parser5parseERNS0_5ValueEPFiRcPvES5_);
    LIB_FUNCTION("LB3jxppxyKU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Parser5parseERNS0_5ValueEPKc);
    LIB_FUNCTION("S5JxQnoGF3E", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6Parser5parseERNS0_5ValueEPKcm);
    LIB_FUNCTION("pCF8Ohwre44", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6String4nposE);
    LIB_FUNCTION("A+FhGENHV-E", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6String5clearEv);
    LIB_FUNCTION("Va5FJ6TmNCw", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6String6appendEPKc);
    LIB_FUNCTION("bREqDgXlqDU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6String6appendEPKcm);
    LIB_FUNCTION("Rz00kiTKypo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6String6appendERKS1_);
    LIB_FUNCTION("PiR0tiTnNbM", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6String6resizeEm);
    LIB_FUNCTION("Z9tyi1-LrQg", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZN3sce4Json6String7reserveEm);
    LIB_FUNCTION("cn9svYGWKDQ", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringaSERKS1_);
    LIB_FUNCTION("9KUZFjI1IxA", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringC1EPKc);
    LIB_FUNCTION("0CAesfH963Q", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringC1ERKS1_);
    LIB_FUNCTION("qSmqLXXCPas", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringC1Ev);
    LIB_FUNCTION("N78gkQzXRL4", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringC2EPKc);
    LIB_FUNCTION("gALFgNfF+9I", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringC2ERKS1_);
    LIB_FUNCTION("eG9E9M6XvTM", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringC2Ev);
    LIB_FUNCTION("cG1VE2HMl6c", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringD1Ev);
    LIB_FUNCTION("Ui7YFnSTCBw", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringD2Ev);
    LIB_FUNCTION("-Wek6mFXOrc", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringpLEh);
    LIB_FUNCTION("aVDQZGAqOo8", "libSceJson2", 1, "libSceJson", 1, 1, _ZN3sce4Json6StringpLEPKc);
    LIB_FUNCTION("GWtzpTKbZ+0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json14InitParameter211getUserDataEv);
    LIB_FUNCTION("BykPm1ajcM0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json14InitParameter212getAllocatorEv);
    LIB_FUNCTION("Pcg6xQUhq9E", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json14InitParameter217getFileBufferSizeEv);
    LIB_FUNCTION("PDkib0zyABg", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json14InitParameter225getSpecialFloatFormatTypeEv);
    LIB_FUNCTION("WXF2ihRF+B8", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5Array3endEv);
    LIB_FUNCTION("bAM9Qwofus0", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5Array4backEv);
    LIB_FUNCTION("rQGJeNjOuUk", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5Array4sizeEv);
    LIB_FUNCTION("bcH5EnFE2xY", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5Array5beginEv);
    LIB_FUNCTION("9uP25i6ipno", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5Array5emptyEv);
    LIB_FUNCTION("zcfZQyM7eAE", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5Array5frontEv);
    LIB_FUNCTION("wcgr5mte7T8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Array8iteratordeEv);
    LIB_FUNCTION("5AZPp99ogrc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Array8iteratorneERKS2_);
    LIB_FUNCTION("iAIYn4oAWvI", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Array8iteratorptEv);
    LIB_FUNCTION("zTwZdI8AZ5Y", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value10getBooleanEv);
    LIB_FUNCTION("DIxvoy7Ngvk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value10getIntegerEv);
    LIB_FUNCTION("sn4HNCtNRzY", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value11getUIntegerEv);
    LIB_FUNCTION("RBw+4NukeGQ", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5Value5countEv);
    LIB_FUNCTION("3qrge7L-AU4", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value7getRealEv);
    LIB_FUNCTION("SHtAad20YYM", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value7getTypeEv);
    LIB_FUNCTION("ONT8As5R1ug", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value8getArrayEv);
    LIB_FUNCTION("0YqYAoO-+Uo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value8getValueEm);
    LIB_FUNCTION("MsMOdxWfbwQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value8getValueERKNS0_6StringE);
    LIB_FUNCTION("Ncel8t2Rrpc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value8toStringERNS0_6StringE);
    LIB_FUNCTION("IlsmvBtMkak", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value9getObjectEv);
    LIB_FUNCTION("epJ6x2LV0kU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5Value9getStringEv);
    LIB_FUNCTION("a-aMMUXqrN0", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5ValuecvbEv);
    LIB_FUNCTION("XlWbvieLj2M", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5ValueixEm);
    LIB_FUNCTION("HwDt5lD9Bfo", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json5ValueixEPKc);
    LIB_FUNCTION("clF7J7N9xXE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json5ValueixERKNS0_6StringE);
    LIB_FUNCTION("ivMCitpSQNk", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json6Object3endEv);
    LIB_FUNCTION("ehN1Biw3tw8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6Object4findERKNS0_6StringE);
    LIB_FUNCTION("fSGHm9RjN5U", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json6Object4sizeEv);
    LIB_FUNCTION("xhAcaIwnrgk", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6Object5beginEv);
    LIB_FUNCTION("i2l3IYvQ9UE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6Object5emptyEv);
    LIB_FUNCTION("ZCd6IYoD3Bc", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6Object8iteratordeEv);
    LIB_FUNCTION("q3YO1c0RqFo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6Object8iteratoreqERKS2_);
    LIB_FUNCTION("+isUKw4zud4", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6Object8iteratorneERKS2_);
    LIB_FUNCTION("EnesV4gXDtE", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6Object8iteratorptEv);
    LIB_FUNCTION("KrKM-5SdQrA", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json6String2atEm);
    LIB_FUNCTION("x4SuxMR26I4", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String4findEcm);
    LIB_FUNCTION("yjsf6SvXA08", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String4findEPKcm);
    LIB_FUNCTION("wwDnDDsQVc0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String4findEPKcmm);
    LIB_FUNCTION("awkeNR6Ukp8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String4findERKS1_m);
    LIB_FUNCTION("UVBkipSD2sw", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json6String4sizeEv);
    LIB_FUNCTION("L1KAkYWml-M", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String5c_strEv);
    LIB_FUNCTION("wM4LO2iK3s8", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String5emptyEv);
    LIB_FUNCTION("t+Dx3eQtBPQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String5rfindEcm);
    LIB_FUNCTION("VpQBXbaHXOo", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String5rfindEPKcm);
    LIB_FUNCTION("A29RHrlfTM0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String5rfindEPKcmm);
    LIB_FUNCTION("-R9mdXBspeQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String5rfindERKS1_m);
    LIB_FUNCTION("EUH+EmT-v9E", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String6lengthEv);
    LIB_FUNCTION("8OCexW9qJgQ", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String6substrEmm);
    LIB_FUNCTION("Yfw63FVJgtU", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String7compareEPKc);
    LIB_FUNCTION("PXIHOuLUN40", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String7compareERKS1_);
    LIB_FUNCTION("q0qeup2r7hA", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6String8capacityEv);
    LIB_FUNCTION("VbFjEs--uiA", "libSceJson2", 1, "libSceJson", 1, 1, _ZNK3sce4Json6StringeqEPKc);
    LIB_FUNCTION("DM43TNBBfb0", "libSceJson2", 1, "libSceJson", 1, 1,
                 _ZNK3sce4Json6StringeqERKS1_);
};

} // namespace Libraries::Json