...
s32 PS4_SYSV_ABI sceCompositorInit() {
    ...
    void* addrs[1] = {(void*)((VAddr)sce_compositor_video_address)};
...
s32 PS4_SYSV_ABI sceCompositorSetCompositeCanvasCommandInC(u8 param_1, AmdGpu::Image* tsharp) {
    LOG_ERROR(Lib_Composite, "(STUBBED) called, param_1: {}", param_1);
    using namespace VideoOut;
    void* addrs[1] = {(void*)((VAddr)tsharp->Address())};

    BufferAttribute attrib = {};
    sceVideoOutSetBufferAttribute(&attrib, PixelFormat::A8R8G8B8Srgb, (u32)TilingMode::Linear, 0,
                                  tsharp->width + 1, tsharp->height + 1, tsharp->width + 1);
...
struct OrbisCompositorPatchDataCanvasTexture {
    uint8_t dstUserDataSlot;
    uint8_t reserved0[3];
    uint32_t canvas_handle;
};

struct OrbisCompositorPatchDataCurrentDisplayBuffer {
    uint8_t mrtSlot;
    uint8_t reserved0[3];
    uint32_t reserved1;
};

union OrbisCompositorPatchData {
    struct OrbisCompositorPatchDataCanvasTexture canvas_texture;
    struct OrbisCompositorPatchDataCurrentDisplayBuffer display_buffer;
    uint64_t asUint;
};

enum OrbisCompositorPatchType {
    ORBIS_COMPOSITOR_PATCH_TYPE_SET_CURRENT_RENDER_TARGET = 0,
    ORBIS_COMPOSITOR_PATCH_TYPE_SET_CURRENT_CANVAS_TEXTURE = 1,
    ORBIS_COMPOSITOR_PATCH_TYPE_SET_CURRENT_CANVAS_TEXTURE_AS_TSHARP_IN_USERDATA = 2
};

struct OrbisCompositorPatchCommand {
    enum OrbisCompositorPatchType type : 8;
    uint8_t reserved[3];
    uint32_t offset;
    union OrbisCompositorPatchData data;
};

s32 PS4_SYSV_ABI sceCompositorSetPatchCommand(OrbisCompositorPatchCommand* patch_begin,
                                              u16 length) {
    LOG_ERROR(Lib_Composite, "(STUBBED) called");
    return ORBIS_OK;
}
